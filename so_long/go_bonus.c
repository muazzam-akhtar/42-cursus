...
/*   Updated: 2021/12/15 16:55:46 by makhtar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	go_up(t_data *vars)
{
	...
	count = vars->rows + 1;
	player_locator(vars);
	if (vars->map[vars->i] == 'P' && vars->map[vars->i - count] == 'M')
	{
		vars->death_triggered = 'W';
		vars->killer = killer_locator(vars);
	}
	else if (vars->map[vars->i] == 'P' && vars->map[vars->i - count] != '1')
	{
...
{
	player_locator(vars);
	if (vars->map[vars->i] == 'P' && vars->map[vars->i + 1] == 'M')
	{
		vars->death_triggered = 'D';
		vars->killer = killer_locator(vars);
	}
	else if (vars->map[vars->i] == 'P' && vars->map[vars->i + 1] != '1')
	{
		movement_right(vars);
		return (1);
	}
