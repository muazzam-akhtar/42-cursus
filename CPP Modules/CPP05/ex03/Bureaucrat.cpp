...
/*   Updated: 2022/06/21 16:12:30 by makhtar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Bureaucrat	&Bureaucrat::operator=(const Bureaucrat &src)
{
	...
	std::cout << "Name is const, so it can't be overwritten" << std::endl << std::endl;
	return (*this);
}
...
void	Bureaucrat::incrementGrade(void)
{
	this->_grade--;
	checkGrade();
	std::cout << GREEN << "Grade incremented successfully from the Bureaucrat" << std::endl << std::endl;
}
...
void	Bureaucrat::decrementGrade(void)
{
	this->_grade++;
	checkGrade();
	std::cout << YELLOW << "Grade decremented successfully from the Bureaucrat" << std::endl << std::endl;
}

void	Bureaucrat::signForm(Form &form) const
{
	form.beSigned(*this) == true ? std::cout << GREEN << this->_name << " signs form " << form.getName()
		<< DEFAULT << std::endl << std::endl : std::cout << RED << this->_name << " couldn't sign form " << form.getName()
		<< " because their grade is not high enough" << DEFAULT << std::endl << std::endl;
}
...
{
	ostr << src.getName();
	ostr << YELLOW << ", bureaucrat grade: " << src.getGrade() << DEFAULT << std::endl << std::endl;
	return (ostr);
}
